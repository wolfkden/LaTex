...
entities, other than proper classes.
...
Overall the atlas dictates the domain and codomain category pair with
topological morphisms, including identities.

An atlas, $\atlas{F}$, implies the pairwise relation of
coordinate frames between metric sets, $\mU{E}_i,\,\mU{E}_j\in\METobj{E}$.
A maifold requires an linear space or
tangent bundle assigned to all interior points on each
cover set $\MU{U}\in\MANobj{M}$.
The morphisms from each tangent space and cover set must
...
The atlas of the $2$-sphere associates two morphisms with
a the same complex space for instance.
...
Tangent space exposes the benefit from the torturous construct of an
atlas relating a maifold with a metric space.
